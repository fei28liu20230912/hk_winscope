...
const in_memory_data_source_1 = require("./in_memory_data_source");
describe('InMemoryDataSource', () => {
    ...
    let dataSource;
    beforeEach(() => {
        dataSource = new in_memory_data_source_1.InMemoryDataSource([...sampleData]); // Use a copy for each test
    });
    test('initialization', () => {
    ...
    describe('filtering', () => {
        ...
        test('is null filter', () => {
            const filters = [{ column: 'value', op: 'is null' }];
            dataSource.notifyUpdate({ direction: 'unsorted' }, filters);
            const result = dataSource.rows;
            expect(result.totalRows).toBe(1);
        ...
        test('is not null filter', () => {
            const filters = [{ column: 'blob', op: 'is not null' }];
            dataSource.notifyUpdate({ direction: 'unsorted' }, filters);
            const result = dataSource.rows;
    ...
    describe('sorting', () => {
        test('sort by string ascending', () => {
            const sortBy = { column: 'name', direction: 'asc' };
        ...
        test('sort by string descending', () => {
            const sortBy = { column: 'name', direction: 'desc' };
            dataSource.notifyUpdate(sortBy, []);
            const result = dataSource.rows;
            expect(result.rows.map((r) => r.name)).toEqual([
                'Trent',
        ...
        test('sort by number ascending (includes nulls)', () => {
            const sortBy = { column: 'value', direction: 'asc' };
            dataSource.notifyUpdate(sortBy, []);
            const result = dataSource.rows;
        ...
        test('sort by number descending (includes nulls and bigint)', () => {
            const sortBy = { column: 'value', direction: 'desc' };
            dataSource.notifyUpdate(sortBy, []);
            const result = dataSource.rows;
        ...
        test('sort by Uint8Array descending (by length)', () => {
            const sortBy = { column: 'blob', direction: 'desc' };
            dataSource.notifyUpdate(sortBy, []);
            const result = dataSource.rows;
        ...
        test('unsorted', () => {
            const sortBy = { direction: 'unsorted' };
    ...
    describe('caching behavior', () => {
        ...
        test('data is reprocessed if sortBy changes', () => {
            ...
            const sortBy1 = { column: 'name', direction: 'asc' };
            const sortBy2 = { column: 'name', direction: 'desc' };
    ...
    test('empty data source', () => {
        const emptyDataSource = new in_memory_data_source_1.InMemoryDataSource([]);
...
//# sourceMappingURL=in_memory_data_source_unittest.js.map
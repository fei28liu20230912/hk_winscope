...
class default_1 {
    ...
    async createHeapProfileTable(trace) {
        await (0, sql_utils_1.createPerfettoTable)(trace.engine, EVENT_TABLE_NAME, `
        SELECT
          MIN(id) as id,
    ...
    `);
        for (const it = result.iter({ upid: query_result_1.NUM }); it.valid(); it.next()) {
            ...
            const track = {
                ...
                track: (0, heap_profile_track_1.createHeapProfileTrack)(trace, uri, EVENT_TABLE_NAME, upid, incomplete),
            };
            trace.tracks.registerTrack(track);
            this.trackMap.set(upid, track);
            const group = trackGroupsPlugin.getGroupForProcess(upid);
            const trackNode = new workspace_1.TrackNode({ uri, title, sortOrder: -30 });

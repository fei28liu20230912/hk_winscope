...
class default_1 {
    ...
    groups = new Map();
    addTrack(ctx, track, groupName, groupCollapsed = true) {
        if (groupName) {
    ...
    async addSliceTrack(ctx, name, query, groupName, columns = [], groupCollapsed = true) {
    ...
    async addCounterTrack(ctx, name, query, groupName, options, groupCollapsed = true) {
        const uri = `/long_battery_tracing_${name}`;
    ...
    async addDeviceState(ctx, features) {
        ...
        const query = (name, track) => this.addBatteryStatsEvent(ctx, name, track, groupName, features);
        const e = ctx.engine;
        ...
        await this.addSliceTrack(ctx, 'Screen state', `SELECT ts, dur, screen_state AS name FROM android_screen_state`, groupName);
        await this.addSliceTrack(ctx, 'Charging', `SELECT ts, dur, charging_state AS name FROM android_charging_states`, groupName);
        await this.addSliceTrack(ctx, 'Suspend / resume', SUSPEND_RESUME, groupName);
        await this.addSliceTrack(ctx, 'Doze light state', `SELECT ts, dur, light_idle_state AS name FROM android_light_idle_state`, groupName);
        await this.addSliceTrack(ctx, 'Doze deep state', `SELECT ts, dur, deep_idle_state AS name FROM android_deep_idle_state`, groupName);
        query('Top app', 'battery_stats.top');
        await this.addSliceTrack(ctx, 'Long wakelocks', `SELECT
        ...
        if (features.has('atom.thermal_throttling_severity_state_changed')) {
            await this.addSliceTrack(ctx, 'Thermal throttling', THERMAL_THROTTLING, groupName);
        }
    }
    async addAtomCounters(ctx) {
       ...
       where ui_name is not null`);
        const countersIt = counters.iter({
            ui_group: query_result_1.STR,
            ui_name: query_result_1.STR,
            ui_unit: query_result_1.STR,
            counter_name: query_result_1.STR,
        });
    ...
    async addAtomSlices(ctx) {
       ...
       order by 1, 2, 3, 4`);
        const slicesIt = sliceTracks.iter({
            atom: query_result_1.STR,
            ui_group: query_result_1.STR,
            ui_name: query_result_1.STR,
            field: query_result_1.STR,
        });
    ...
    async addDayExplorerCounters(ctx, table, groupName, limit) {
        const e = ctx.engine;
        await e.query(`INCLUDE PERFETTO MODULE
          google3.wireless.android.telemetry.trace_extractor.modules.day_explorer.perfetto_ui_blames`);
        const counters = await e.query(`select track_name, cast(round(total_energy_uws / 3600000) as int) as energy_mwh
      from ${table}_totals order by total_energy_uws desc limit ${limit}`);
        const countersIt = counters.iter({
            track_name: query_result_1.STR,
            energy_mwh: query_result_1.LONG,
        });
        for (; countersIt.valid(); countersIt.next()) {
            const opts = { unit: 'mW', yRangeSharingKey: `day-explorer-${table}` };
            await this.addCounterTrack(ctx, `${countersIt.track_name} - ${countersIt.energy_mwh} mWh`, `select ts, power_mw as value
         from ${table}
         where track_name = '${countersIt.track_name}'`, groupName, opts);
        }
    }
    async addDayExplorerBehaviors(ctx, groupName) {
        const e = ctx.engine;
        await e.query(`INCLUDE PERFETTO MODULE
          google3.wireless.android.telemetry.trace_extractor.modules.day_explorer.perfetto_ui_blames`);
        await this.addSliceTrack(ctx, 'Day Explorer Behaviors', `select ts, dur, behavior as name from day_explorer_behaviors`, groupName, [], false);
    }
    async addNetworkSummary(ctx, features) {
        ...
        if (features.has('net.wifi')) {
            ...
            const it = result.iter({ pkg: query_result_1.STR });
    ...
    async addModemRil(ctx, features) {
    ...
    async addModemTeaData(ctx) {
        ...
        const countersIt = counters.iter({ name: query_result_1.STR });
        for (; countersIt.valid(); countersIt.next()) {
        ...
        const slicesIt = slices.iter({ track_name: query_result_1.STR });
        for (; slicesIt.valid(); slicesIt.next()) {
    ...
    async addKernelWakelocks(ctx) {
        const e = ctx.engine;
        await e.query(`INCLUDE PERFETTO MODULE android.kernel_wakelocks;`);
        const result = await e.query(`SELECT DISTINCT name, type FROM android_kernel_wakelocks`);
        const it = result.iter({ name: query_result_1.STR, type: query_result_1.STR });
        for (; it.valid(); it.next()) {
            await this.addCounterTrack(ctx, it.name, `SELECT ts, dur, held_ratio * 100 AS value
         FROM android_kernel_wakelocks
         WHERE name = "${it.name}"`, 'Kernel Wakelock Summary', { yRangeSharingKey: 'kernel_wakelock', unit: '%' });
        }
    }
    async addKernelWakelocksStatsd(ctx, features) {
        ...
        const it = result.iter({ wakelock_name: query_result_1.STR });
        for (; it.valid(); it.next()) {
            await this.addCounterTrack(ctx, `${it.wakelock_name} (statsd)`, `select ts, dur, value
         from kernel_wakelocks_statsd
         where wakelock_name = "${it.wakelock_name}"`, groupName, { yRangeSharingKey: 'kernel_wakelock_statsd', unit: '%' });
        }
    }
    async addWakeups(ctx, features) {
      ...
      having sum_dur > 600e9`);
        const it = result.iter({ item: query_result_1.STR });
    ...
    async addHighCpu(ctx, features) {
        ...
        const it = result.iter({ pkg: query_result_1.STR, cluster: query_result_1.STR });
        for (; it.valid(); it.next()) {
    ...
    async findFeatures(e) {
        ...
        const addFeatures = async (q) => {
            ...
            const it = result.iter({ feature: query_result_1.STR });
            for (; it.valid(); it.next()) {
    ...
    DAY_EXPLORER_TABLES = {
        ...
        day_explorer_screen_off_category_package_level_1: 'DE Screen Off: Cat / Pkg / L1',
        day_explorer_screen_off_category_package_level_1_level_2: 'DE Screen Off: Cat / Pkg / L1 / L2',
        day_explorer_screen_on_category: 'DE Screen On: Category',
        day_explorer_screen_on_category_package: 'DE Screen On: Cat / Pkg',
        day_explorer_screen_on_category_package_level_1: 'DE Screen On: Cat / Pkg / L1',
        day_explorer_screen_on_category_package_level_1_level_2: 'DE Screen On: Cat / Pkg / L1 / L2',
    };
    async addDayExplorerCommand(ctx, features) {
        if (features.has('google3')) {
            ctx.commands.registerCommand({
                id: 'perfetto.DayExplorerBlamesByCategory',
                name: 'Add tracks: Day Explorer',
                callback: async () => {
                    const limitStr = await ctx.omnibox.prompt('Maximum results per group');
                    ...
                    for (const [table, desc] of Object.entries(this.DAY_EXPLORER_TABLES)) {

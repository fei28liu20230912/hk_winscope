...
class LogPanel {
    ...
    view({ attrs }) {
        ...
        return (0, mithril_1.default)(details_shell_1.DetailsShell, {
            ...
            buttons: (0, mithril_1.default)(LogsFilters, {
                trace: attrs.trace,
                cache: attrs.cache,
    ...
    renderRows(hasMachineIds, hasProcessNames) {
        if (!this.entries) {
            return [];
        }
...
class LogsFilters {
    ...
    renderFilterPanel(attrs) {
        ...
        const options = attrs.cache.uniqueMachineIds.map((uMachineId) => {
            return {
                id: String(uMachineId),
                name: `Machine ${uMachineId}`,
                checked: !machineExcludeList.some((excluded) => excluded === uMachineId),
            };
        });
        return (0, mithril_1.default)(multiselect_1.PopupMultiSelect, {
            ...
            options,
            onChange: (diffs) => {
                const newList = new Set(machineExcludeList);
...
async function updateLogEntries(engine, span, pagination) {
    ...
    const it = rowsResult.iter({
        ...
        machineId: query_result_1.NUM_NULL,
    });

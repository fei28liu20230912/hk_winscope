...
async function getMachineIds(engine) {
    ...
    const result = await engine.query(`SELECT DISTINCT(machine_id) FROM cpu ORDER BY machine_id`);
    const machineIds = [];
    const it = result.iter({ machine_id: query_result_1.NUM_NULL });
...
class default_1 {
    ...
    async onTraceLoad(ctx) {
        ...
        ctx.tabs.registerTab({
            ...
            content: {
                render: () => (0, mithril_1.default)(logs_panel_1.LogPanel, { filterStore, cache, trace: ctx }),

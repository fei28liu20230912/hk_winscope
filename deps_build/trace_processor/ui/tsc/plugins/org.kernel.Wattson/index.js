    ...
    `);
        if (checkValue.firstRow({ numRows: query_result_1.NUM }).numRows > 0) {
            ...
            ctx.tracks.registerTrack({
                ...
                title,
                tags: {
                    kind: track_kinds_2.SLICE_TRACK_KIND,
                },
                track,
            });
...
class GpuSubsystemEstimateTrack extends base_counter_track_1.BaseCounterTrack {
    queryKey;
    constructor(trace, uri, queryKey) {
        super(trace, uri);
        this.queryKey = queryKey;
...
async function addWattsonCpuElements(ctx, group) {
    ...
    ctx.selection.registerAreaSelectionTab((0, aggregation_panel_1.createWattsonAggregationToTabAdaptor)(ctx, new estimate_aggregator_1.WattsonEstimateSelectionAggregator()));
    ctx.selection.registerAreaSelectionTab((0, aggregation_panel_1.createWattsonAggregationToTabAdaptor)(ctx, new thread_aggregator_1.WattsonThreadSelectionAggregator()));
    ctx.selection.registerAreaSelectionTab((0, aggregation_panel_1.createWattsonAggregationToTabAdaptor)(ctx, new process_aggregator_1.WattsonProcessSelectionAggregator()));
    ctx.selection.registerAreaSelectionTab((0, aggregation_panel_1.createWattsonAggregationToTabAdaptor)(ctx, new package_aggregator_1.WattsonPackageSelectionAggregator()));
}
async function addWattsonGpuElements(ctx, group) {
    ...
    ctx.tracks.registerTrack({
        ...
        tags: {
            kind: track_kinds_1.GPUSS_ESTIMATE_TRACK_KIND,
            wattson: 'Gpu',
            groupName: `Wattson`,
        },
    });

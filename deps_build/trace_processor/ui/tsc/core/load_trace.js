...
async function createEngine(app, engineId) {
    ...
    engine.onResponseReceived = () => raf_scheduler_1.raf.scheduleFullRedraw();
...
async function getCpus(engine) {
    const cpus = [];
    const queryRes = await engine.query(`select ucpu, cpu, ifnull(machine_id, 0) as machine from cpu`);
    for (const it = queryRes.iter({ ucpu: query_result_1.NUM, cpu: query_result_1.NUM, machine: query_result_1.NUM }); it.valid(); it.next()) {
        cpus.push(new multi_machine_trace_1.Cpu(it.ucpu, it.cpu, it.machine));
    }
    return cpus;
}

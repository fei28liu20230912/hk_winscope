...
const DATASET_SEARCH = feature_flags_1.featureFlags.register({
    ...
    description: '[Experimental] use dataset for search, which allows searching all tracks with a matching dataset. Might be slower than normal search.',
    defaultValue: false,
});
class SearchManagerImpl {
    ...
    async executeDatasetSearch() {
        ...
        const generation = this._searchGeneration;
        const allResults = await (0, dataset_search_1.searchTrackEvents)(engine, trackManager.getAllTracks(), this._searchText);
